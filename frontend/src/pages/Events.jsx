...
export default function EventsPage({ user, openAuth }) {
  ...
  const eventEnd = new Date(event.endDate || `${event.date}T${event.endTime}`);
      ...
      const matchesDate = !selectedDate || (event.date === selectedDate || event.startDate?.startsWith(selectedDate));

      return matchesSearch && matchesCategory && matchesStatus && matchesTime && matchesDate;
    });
  }, [searchQuery, selectedCategory, statusFilter, timeFilter, selectedDate, eventsData]);
  const handleRegister = async (eventId) => {
    if (!user) {
      openAuth?.("login");
      return;
    }
    const event = eventsData.find((item) => item._id === eventId || item.id === eventId);
    if (!event) {
  ...
  const handleCancelRegistration = async (eventId) => {
    ...
    if (registration.status === 'rejected') {
      setToast({ type: "error", message: "Không thể hủy đăng ký đã bị từ chối." });
      return;
    }
              ...
              {TIME_FILTERS.map(({ value, label }) => (
                <button
                  ...
                  className={`rounded-lg px-3 py-1 text-sm font-medium ${
                    timeFilter === value ? "bg-primary-100 text-primary-700" : "bg-surface-muted text-text-secondary hover:bg-gray-200"
                  }`}
                >
                  {label}
                </button>
        ...
        <section className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {filteredEvents.length === 0 ? (
            <div className="col-span-full text-center py-12">
              <p className="text-text-muted text-lg">Không tìm thấy sự kiện nào phù hợp</p>
            </div>
          ) : (
            filteredEvents.map((event, idx) => {
              const eventId = event._id || event.id;
              const registration = userRegistrations[eventId];
              const isFull = (event.currentParticipants ?? event.registered ?? 0) >= (event.maxParticipants ?? event.slots ?? 0);
              const isApproved = event.status === 'approved' || !event.status; // Default to approved if no status

              // Determine button state
              let buttonState = 'register'; // register, pending, accepted, rejected, full
              if (registration) {
                ...
                <div className="flex flex-col flex-1 p-5 space-y-3">
                  ...
                  <div className="space-y-2 text-sm text-text-secondary">
                    <div className="flex items-center gap-2">
                      ...
                      <Clock className="h-4 w-4 text-text-muted ml-2" />
                      <span>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {event.tags.map((tag) => (
                      <span key={tag} className="rounded-md bg-surface-muted px-2 py-0.5 text-xs text-text-secondary">
                        #{tag}
                      </span>
                    ))}
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2 pt-3 border-t border-gray-200">
                    ...
                    {(!user || isVolunteer) && isApproved && (
                      <>
                        {buttonState === 'pending' ? (
                          <div className="flex-1 flex gap-2">
                            ...
                            <button
                              ...
                              className="rounded-lg border border-error-200 px-3 py-2 text-sm font-semibold text-error-600 transition hover:bg-error-50"
                              title="Hủy đăng ký"
                            >
                        ...
                        <button
                          onClick={() => alert(`Xem chi tiết ${eventId}`)}
                    ...
                    {isAdmin && (
                      <>
                        {event.status === "pending" && (
                          <>
                            <button
                              ...
                              className="flex items-center gap-1 rounded-lg bg-success-500 px-3 py-2 text-sm font-semibold text-white hover:bg-success-600"
                            >
                            ...
                            <button
                              ...
                              className="flex items-center gap-1 rounded-lg bg-error-500 px-3 py-2 text-sm font-semibold text-white hover:bg-error-600"
                            >
                              <XCircle className="h-4 w-4" />
                              Từ chối
                            </button>
                        ...
                        <button
                          onClick={() => alert(`Xem chi tiết quản trị ${eventId}`)}
                          className="rounded-lg border border-gray-200 px-3 py-2 text-sm font-semibold text-text-secondary hover:bg-surface-muted"
                        >
                          Chi tiết
                        </button>
