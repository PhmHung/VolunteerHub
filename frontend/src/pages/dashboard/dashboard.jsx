...
const Dashboard = () => {
  ...
  const allEvents = useMemo(() => {
    return events.map((event) => ({
      ...
      imageUrl: event.image || 'https://images.unsplash.com/photo-1559027615-cd4628902d4a?w=400',
  ...
  const eventsWithCoordinates = useMemo(() => {
    return allEvents.filter((e) => e.coordinates).map((event) => ({
      ...event,
      city: event.city || event.location?.split(',').pop()?.trim() || 'Không rõ',
    }));
  }, [allEvents]);
  ...
  useEffect(() => {
    if (!selectedLocation && eventsWithCoordinates.length > 0) {
      ...
      setActiveEventId(eventsWithCoordinates[0].id || eventsWithCoordinates[0]._id);
  ...
  const stats = useMemo(() => {
    ...
    const cities = new Set(
      allEvents.map((e) => e.city || e.location?.split(',').pop()?.trim()).filter(Boolean)
    );
    return {
...
const InteractiveCalendar = ({ currentDate, setCurrentDate, selectedDate, setSelectedDate, events }) => {
          ...
          return (
            <button
              ...
              className={`
                ...
                ${isSelected(day) && !isToday(day) ? 'bg-blue-100 text-blue-700 ring-2 ring-blue-500' : ''}
                ...
                <span className="absolute bottom-1 left-1/2 -translate-x-1/2 flex gap-0.5">
                  {evtCount <= 3 ? (
                    [...Array(evtCount)].map((_, i) => (
                      <span key={i} className={`h-1 w-1 rounded-full ${isToday(day) ? 'bg-white' : 'bg-blue-500'}`} />
                    ))
                  ) : (
                    <span className={`text-[10px] font-bold ${isToday(day) ? 'text-white' : 'text-blue-600'}`}>
                      {evtCount}
                    </span>
                  )}
                </span>
    ...
    <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <h3 className="text-lg font-bold text-gray-900">{formatFullDate(date)}</h3>
        </div>
      ...
      {events.length > 0 ? (
        <div className="space-y-3">
          {events.map((event) => (
            ...
            >
              <img src={event.imageUrl} alt={event.title} className="h-14 w-14 rounded-lg object-cover" />
    ...
    <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }}>
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <h3 className="text-lg font-bold text-gray-900">Sự kiện sắp diễn ra</h3>
        </div>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-3 mt-1 text-xs text-gray-500">
                    ...
                    <span className="flex items-center gap-1">
                      ...
                      {event.currentParticipants || 0}/{event.maxParticipants || event.volunteersNeeded}
                    </span>
                  </div>
                </div>
              </Link>
    ...
    <div className="p-5">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mb-4">
        <div>
          ...
          <h3 className="text-lg font-bold text-gray-900">Bản đồ sự kiện tình nguyện</h3>
        </div>
              ...
              return (
                <li
                  ...
                  className={`flex cursor-pointer items-center gap-3 rounded-2xl border p-3 transition hover:-translate-y-0.5 ${
                    isActive ? 'border-blue-300 bg-blue-50 shadow-md' : 'border-gray-100 bg-gray-50 hover:bg-white'
                  }`}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      <span className="inline-flex items-center gap-1 rounded-full bg-white px-2 py-0.5 text-xs font-medium text-gray-600 border">
                        ...
                        {event.currentParticipants || event.registered || 0}/{event.maxParticipants || event.volunteersNeeded || '∞'}
                      </span>
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
...
const InteractiveLeafletMap = ({
  ...
  return (
    ...
    >
      ...
      />
      {showSelectionMarker && (
        <Marker position={[selection.lat, selection.lng]} icon={DEFAULT_MARKER_ICON}>
        ...
        <Marker
          ...
          <Popup>
            <div className="min-w-[200px]">
              ...
              <p className="text-xs text-gray-600">{event.location || event.city}</p>
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500">Đang được quan tâm nhiều nhất</p>
          </div>
        </div>
        <Link to="/events" className="text-sm text-blue-600 hover:text-blue-700 font-medium flex items-center gap-1">
          Xem tất cả <ArrowRight className="h-4 w-4" />
          ...
          {events.map((event, index) => (
            ...
            >
              ...
              >
                ...
                <img src={event.imageUrl} alt={event.title} className="w-full h-28 object-cover rounded-xl mb-3" />
        ...
        <div className="space-y-3">
          {events.map((event, index) => (
            <motion.div
              ...
              transition={{ delay: index * 0.1 }}
            >
              <Link to="/events" className="flex items-center gap-3 p-2 rounded-xl hover:bg-blue-50 transition group">
                <div className="relative">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-500">
                    {formatDate(event.startDate)} • {event.city || event.location?.split(',').pop()?.trim()}
                  </p>
                </div>
                <span className="text-xs font-medium text-blue-600 bg-blue-50 px-2 py-1 rounded-full">Mới</span>
              </Link>
            </motion.div>
          ))}
        </div>
