...
const Dashboard = ({ user }) => {
  ...
  const upcomingEventCount = useMemo(() => {
    ...
    return eventSummaries.filter((event) =>
      event.eventDate ? event.eventDate >= now : true
    ).length;
  }, [eventSummaries]);
  ...
  const upcomingHighlights = useMemo(() => {
    const now = new Date();
    const upcoming = eventSummaries.filter((event) =>
      event.eventDate ? event.eventDate >= now : true
    );
  ...
  const scheduleHighlights = useMemo(() => {
    return eventSummaries
      .slice(0, Math.min(3, eventSummaries.length))
      .map((event) => ({
        id: event.id,
  ...
  const calendarData = useMemo(
    () => buildCalendarData(eventSummaries),
    [eventSummaries]
  );
    ...
    <div className='mt-6 space-y-5'>
      {events.length > 0 ? (
        ...
        <p className='rounded-2xl border border-dashed border-slate-200 bg-slate-50 p-6 text-sm text-slate-600'>
          Hiện chưa có sự kiện nào phù hợp. Hãy khám phá các sự kiện khác trong
          danh sách bên dưới.
        </p>
      )}
    </div>
...
const TicketCard = ({ event }) => (
  <article className='grid gap-6 rounded-[24px] border border-slate-100 bg-slate-50/70 p-5 shadow-sm transition hover:-translate-y-1 hover:bg-white md:grid-cols-[1.4fr_auto] md:items-center'>
    <div className='flex flex-col gap-5 md:flex-row md:items-center md:gap-6'>
      ...
      <div className='space-y-3'>
        ...
        <p className='text-xs text-slate-500'>
          Đã đăng ký: {event.registered.toLocaleString("vi-VN")} tình nguyện
          viên
        </p>
    ...
    <div className='flex flex-col items-center justify-center gap-4 rounded-[20px] border-t border-dashed border-slate-200 pt-4 text-sm text-slate-500 md:border-l md:border-t-0 md:pl-6'>
      <img
        src={event.qrCode}
        alt={`QR cho ${event.name}`}
        className='h-28 w-28 rounded-xl bg-white p-2 shadow'
      />
      ...
      <header className='flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between'>
        <div>
          ...
          <h3 className='text-lg font-semibold text-slate-900'>
            Những sự kiện được săn vé nhiều nhất
          </h3>
        </div>
    ...
    <section className='rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-100'>
      <header className='flex items-center justify-between'>
        <div>
          ...
          <h3 className='text-lg font-semibold text-slate-900'>
            Đừng bỏ lỡ các sự kiện sắp diễn ra
          </h3>
        </div>
        <button className='text-sm font-semibold text-indigo-600 hover:text-indigo-700'>
          Xem thêm
    ...
    <section className='rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-100'>
      <header className='flex items-center justify-between'>
        <div>
          ...
          <h3 className='text-lg font-semibold text-slate-900'>
            Những địa điểm tổ chức được yêu thích
          </h3>
        </div>
        <button className='text-sm font-semibold text-indigo-600 hover:text-indigo-700'>
          Xem thêm
      ...
      <ul className='mt-5 space-y-3'>
        {schedule.length > 0 ? (
          ...
          <li className='rounded-2xl border border-dashed border-slate-200 bg-slate-50 p-6 text-sm text-slate-600'>
            Chưa có sự kiện nào trong lịch trình. Hãy cập nhật dữ liệu để hiển
            thị danh sách mới.
          </li>
        )}
      </ul>
              ...
              return (
                <span
                  ...
                  className={`flex h-9 items-center justify-center rounded-full border border-transparent text-slate-600 ${
                    highlight
                      ? `${highlight.color} text-white font-semibold`
                      : "bg-white"
                  }`}>
                  {day}
                </span>
