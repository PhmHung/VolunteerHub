...
import { Calendar, MapPin, MessageSquare, ArrowLeft, Search, Info, Clock, Image } from 'lucide-react';
            ...
            <div className="lg:col-span-3">
                ...
                {activeTab === 'about' && (
                    <div className="card p-6">
                        <h2 className="text-xl font-bold mb-4 text-text-main">Giới thiệu chi tiết</h2>
                        <p className="text-text-secondary whitespace-pre-line leading-relaxed">{event.description}</p>
                ...
                {activeTab === 'media' && (
                    <div className="card p-12 text-center">
                        ...
                        <h3 className="text-lg font-medium text-text-main">Thư viện ảnh</h3>
                        <p className="text-text-muted mt-1">Chưa có hình ảnh nào được tải lên.</p>
                    </div>
                )}
            </div>
        </div>
...
const Media = ({ user }) => {
  ...
  useEffect(() => {
    ...
    if (user.role === 'manager') {
      ...
      const acceptedRegs = myRegistrations.filter(reg => reg.status === 'accepted');
      
      // Get event details for these registrations
      const events = acceptedRegs.map(reg => {
        const eventId = reg.eventId?._id || reg.eventId;
        const event = allEvents.find(e => e._id === eventId || e.id === eventId);
    ...
    <div className='w-full md:flex h-screen overflow-hidden bg-surface-50'>
      <div className="flex-1 overflow-y-auto bg-surface-50/50">
        <div className="max-w-5xl mx-auto p-6 lg:p-10">
          <div className="mb-10">
            <h1 className="text-3xl font-bold text-text-main mb-2">Cộng đồng của tôi</h1>
            <p className="text-text-secondary">Các sự kiện bạn đã tham gia và được chấp nhận</p>
          </div>
          ...
          {myEvents.length === 0 ? (
            <div className="text-center py-16 card">
              ...
              <h3 className="text-xl font-bold text-text-main mb-2">Chưa có cộng đồng nào</h3>
              <p className="text-text-secondary max-w-md mx-auto">Bạn chưa có sự kiện nào được duyệt tham gia. Hãy đăng ký sự kiện mới và chờ quản trị viên xét duyệt nhé!</p>
            </div>
          ) : (
                    ...
                    <div className="flex flex-col md:flex-row">
                        ...
                        <div className="md:w-2/5 relative h-64 md:h-auto overflow-hidden">
                            ...
                            <div className="absolute top-4 left-4 bg-white/95 backdrop-blur-md px-4 py-2 rounded-2xl shadow-lg">
                                <p className="text-xs font-bold text-text-secondary uppercase tracking-wider mb-0.5">Tháng {new Date(event.startDate).getMonth() + 1}</p>
                                <p className="text-2xl font-black text-text-main leading-none">{new Date(event.startDate).getDate()}</p>
                            </div>
                            ...
                            <div>
                                <div className="flex justify-between items-start mb-4">
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="text-xs font-bold text-success-600 uppercase tracking-wider">Đã xác nhận</span>
                                    </div>
                                </div>
