...
export default function PieStat({ title = 'Pie Stat', data = [], height = 180 }) {
  return (
    <div className="bg-surface-base p-4 rounded-xl shadow-sm h-full">
      <div className="mb-3">
        ...
        {data.map((d, i) => (
          <li key={i} className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <span style={{ width: 10, height: 10, backgroundColor: d.color || '#10b981', display: 'inline-block', borderRadius: 3 }} />
              <span>{d.name}</span>
            </div>
            <span className="font-semibold text-text-main">{d.value}%</span>
          </li>
        ))}
      </ul>
    </div>
