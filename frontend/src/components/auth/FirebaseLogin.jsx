import { signInWithPopup } from "https://www.gstatic.com/firebasejs/10.1.0/firebase-auth.js";
<<<<<<< Updated upstream:frontend/src/components/auth/FirebaseLogin.jsx
import { auth, googleProvider } from "../../../../backend/config/firebase.js";

=======
import { auth, googleProvider } from "../../../backend/config/firebase.js";
import axios from "axios";
>>>>>>> Stashed changes:frontend/src/components/FirebaseLogin.jsx

const GOOGLE_ICON =
  "data:image/png;base64,";

export default function FirebaseLogin({ onSuccess }) {

  const handleGoogleLogin = async () => {
    if (!auth) return console.error("Firebase auth not initialized");

    try {
      // 1. Đăng nhập Google qua Firebase
      const result = await signInWithPopup(auth, googleProvider);
      const idToken = await result.user.getIdToken();

      // 2. Gửi token Firebase lên backend để xác thực
      const res = await axios.post(
        "http://localhost:5000/api/auth/firebase-login",
        { idToken }
      );

      const data = res.data;

      // 3. Nếu backend trả token -> lưu vào localStorage
      if (data?.token) {
        localStorage.setItem("token", data.token);
        if (data.picture) {
          localStorage.setItem("picture", data.picture);
        }
        onSuccess?.(data);
      } else {
        console.error("Google auth response missing token", data);
      }
    } catch (err) {
      if (err.response) {
        // Backend trả lỗi status 400-500
        console.error("Backend error:", err.response.data.message);
        alert(err.response.data.message); // hoặc setError(...)
      } else {
        // Lỗi mạng / Firebase lỗi
        console.error("Google login error:", err);
      }
    }
  };


  return (
    <button
      onClick={handleGoogleLogin}
      className="mt-7 w-full flex items-center justify-center bg-white border border-gray-300 rounded-lg px-4 py-2 shadow hover:shadow-md transition"
    >
      <img src={GOOGLE_ICON} alt="Google" className="w-6 h-6 mr-2" />
      <span className="text-gray-700 font-medium">Sign in with Google</span>
    </button>
  );
}
